...
class Agent(object):
    ...
    def build_computation_graph(self):
        ...
        self.critic_prediction = tf.squeeze(build_mlp(
                                ...
                                size=self.size))
        # Difference with policy gradient, target is not a single episode's sum of rewards
    ...
    def estimate_advantage(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        # YOUR CODE HERE
        v_n1   = self.sess.run(self.critic_prediction, feed_dict = {self.sy_ob_no: next_ob_no})
        q_n    = re_n + self.gamma * (v_n1 * (1 - terminal_n))
    ...
    def update_critic(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        for n in range(self.num_grad_steps_per_target_update * self.num_target_updates):
            # Recompute the target value for every target update steps
            if n % self.num_grad_steps_per_target_update == 0:
                v_n1   = self.sess.run(self.critic_prediction, feed_dict = {self.sy_ob_no: next_ob_no})
                target = re_n + self.gamma * (v_n1 * (1 - terminal_n))
