...
class QLearner(object):
  ...
  def __init__(
    ...
    # YOUR CODE HERE
    # get Q-value with given state and next state
    self.q_t  = q_func(obs_t_float, self.num_actions, scope="q_net", reuse=False)
    self.q_t1 = q_func(obs_tp1_float, self.num_actions, scope="q_net", reuse=True)
    self.target_q_t1 = q_func(obs_tp1_float, self.num_actions, scope="target_q_net", reuse=False)
    
    # get Q-value based on the chosen action
    max_q_t = tf.reduce_sum(self.q_t * tf.one_hot(self.act_t_ph, self.num_actions), axis=1)
    
    # get maximum Q-value in the next state
    if double_q == True:
        q_t1_action = tf.argmax(self.q_t1, 1)
        target_max_q_t1 = tf.reduce_sum(self.target_q_t1 * tf.one_hot(q_t1_action, self.num_actions), axis = 1)
    else:
        target_max_q_t1 = tf.reduce_max(self.target_q_t1,1)
    
    # Compute the Bellman error
    target = self.rew_t_ph + (1 - self.done_mask_ph) * gamma * target_max_q_t1
    self.total_error = tf.reduce_mean(huber_loss((target - max_q_t)))    
                                      
    # Get the Variables for each network
  ...
  def step_env(self):
    ...
    #####
    # Store a frame into the buffer
    frame_idx = self.replay_buffer.store_frame(self.last_obs)
    
    # Exploration rate (t,value) = (0, 1.0),(1e6, 0.1),(num_iterations / 2, 0.01)        
    eps = self.exploration.value(self.t)
    
    if self.model_initialized == False or random.random() <= eps:
        action = np.random.randint(0, self.num_actions)
    
    else:
        # Context from previous frames
        observations = np.expand_dims(self.replay_buffer.encode_recent_observation(),0)
        q_values = self.session.run(self.q_t, feed_dict = {self.obs_t_ph: observations})
        action = np.argmax(q_values[0])
    
    # Step envrionment based on the selected action
    next_obs, reward, done, _ = self.env.step(action)
    
    # Store step infomation to the buffer
  ...
  def log_progress(self):
    ...
    if self.t % self.log_every_n_steps == 0 and self.model_initialized:
      ...
      logs = []
      log = {'Timestep':self.t,
              'mean reward (100 episodes)': self.mean_episode_reward,
              'best mean reward' : self.best_mean_episode_reward,
              'episodes':len(episode_rewards)}
      
      if os.path.exists('./logs/' + self.rew_file):
        with open('./logs/' + self.rew_file, 'rb') as f:
            logs = pickle.load(f)
      
      logs.append(log)
      with open('./logs/' + self.rew_file, 'wb') as f:
